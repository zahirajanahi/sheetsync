    ...
    if (incoherenceTypeFilter !== 'all' && statusFilter !== 'Correct') {
      switch(incoherenceTypeFilter) {
        ...
        case 'hs50':
          ...
          break;
        case 'heures':
          incoherenceMatch = Math.abs(item.difference - item.primeRendement) > 0.01; // Modified to account for prime rendement
          break;
        default:
 ...
 <nav className="backdrop-blur-xl bg-black/40 border-b border-white/10 shadow-lg sticky top-0 z-10">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="bg-[#222222]/80 p-8 rounded-lg border border-gray-800 shadow-xl">
          <h1 className="text-2xl font-bold text-white mb-6">Traitement Pointage vs Paie Grand Ceram</h1>
  ...
  (item.difference - item.primeRendement) > 0 ? 'text-red-400' : 
  (item.difference - item.primeRendement) < 0 ? 'text-red-800' : 'text-gray-400'
}`}>
  {(item.difference - item.primeRendement)?.toFixed(2) || '0.00'}
</td>
                          ...
                          <tr className="bg-gray-800/50">
                            <td colSpan="7" className="px-6 py-4">
                              <div className="space-y-3">
                                ...
                                {/* Hours comparison - Modified to account for prime rendement */}
<div className={`p-3 rounded-lg ${
  Math.abs((item.heuresTravaillees - item.primeRendement) - item.heuresPayees) <= 0.01 
    ? 'bg-green-900/20' 
    : 'bg-red-900/20'
}`}>
  <div className="flex items-center">
    ...
    <span className="font-medium text-sm">
      Validation Heures (avec Prime Rendement): 
    </span>
    <span className={`ml-2 text-sm ${
      Math.abs((item.heuresTravaillees - item.primeRendement) - item.heuresPayees) <= 0.01 
        ? 'text-green-400' 
  ...
  <div className="mt-1 text-xs text-gray-300">
    Prime Rendement: {item.primeRendement?.toFixed(2) || '0.00'}
  </div>
  <div className="mt-1 text-xs text-gray-300">
    Heures Pointage - Prime Rendement: {(item.heuresTravaillees - item.primeRendement)?.toFixed(2) || '0.00'}
  </div>
  <div className="mt-1 text-xs text-gray-300">
  ...
  {Math.abs((item.heuresTravaillees - item.primeRendement) - item.heuresPayees) > 0.01 && (
    <div className="mt-1 text-xs text-red-300">
      <Info className="inline w-3 h-3 mr-1" />
      Différence de {Math.abs((item.heuresTravaillees - item.primeRendement) - item.heuresPayees).toFixed(2)} heures
    </div>
  )}
</div>
