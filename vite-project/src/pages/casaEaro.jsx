...
const casaEaro = () => {
  ...
  // Filter results based on status and CIN only (removed incoherence type filter)
     ...
     <nav className="backdrop-blur-xl bg-black/40 border-b border-white/10 shadow-lg sticky top-0 z-10">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="bg-[#222222]/80 p-8 rounded-lg border border-gray-800 shadow-xl">
          <h1 className="text-2xl font-bold text-white mb-6">Traitement Pointage vs Paie CasaEaro</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500 mt-1">Format attendu: CIN, Heures Travaillées, HS 125%, HS 150%</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500 mt-1">Format attendu: CIN, Jrs/Hrs, HS 25, HS 50</p>
              </div>
            </div>
      ...
      <p className="text-2xl font-bold text-red-400">
        {results.results.filter(r => 
          r.paieValidations?.amoCnssCheck?.status === 'Employé non déclaré').length}
              ...
              {/* Filters - Removed incoherence type filter */}
        ...
        {item.transpStatus !== undefined && (
          <div className={`p-3 rounded-lg ${
            item.transpStatus === 'Correct' ? 'bg-green-900/20' : 'bg-red-900/20'
          }`}>
            <div className="flex items-center">
              {item.transpStatus === 'Correct' ? (
                <Check className="w-4 h-4 text-green-400 mr-2" />
              ) : (
                <X className="w-4 h-4 text-red-400 mr-2" />
              )}
              ...
              <span className={`ml-2 text-sm ${
                item.transpStatus === 'Correct' ? 'text-green-400' : 'text-red-400'
              }`}>
                {item.transpStatus}
              </span>
            </div>

            {item.transpPointage === undefined && item.transpPaie === undefined ? (
              <div className="text-xs text-gray-500 italic">
                Données Transport non disponibles pour cet employé
              </div>
            ) : (
              <>
                {item.transpPointage !== undefined && (
                  <div className="mt-1 text-xs text-gray-300">
                    Transport Pointage: {item.transpPointage.toFixed(2)} MAD
                  </div>
                )}
                {item.transpPaie !== undefined && (
                  <div className="mt-1 text-xs text-gray-300">
                    Transport Paie: {item.transpPaie.toFixed(2)} MAD
                  </div>
                )}
              </>
            )}

            {item.transpStatus === 'Incohérence' && (
              <div className="mt-1 text-xs text-red-300">
                <Info className="inline w-3 h-3 mr-1" />
                Les valeurs devraient être identiques
                                ...
                                {/* HS 125% comparison */}
                                <div className={`p-3 rounded-lg ${
                                  item.hs125Status === 'Correct' ? 'bg-green-900/20' : 'bg-red-900/20'
                                }`}>
                                  <div className="flex items-center">
                                    {item.hs125Status === 'Correct' ? (
                                      <Check className="w-4 h-4 text-green-400 mr-2" />
                                    ) : (
                                      <X className="w-4 h-4 text-red-400 mr-2" />
                                    )}
                                    <span className="font-medium text-sm">
                                      HS 125% Pointage vs HS 25 Paie: 
                                    </span>
                                    <span className={`ml-2 text-sm ${
                                      item.hs125Status === 'Correct' ? 'text-green-400' : 'text-red-400'
                                    }`}>
                                      {item.hs125Status}
                                    </span>
                                  </div>

                                  {item.hs125Pointage === undefined && item.hs25Paie === undefined ? (
                                    <div className="text-xs text-gray-500 italic">
                                      Données HS 125%/25% non disponibles pour cet employé
                                    </div>
                                  ) : (
                                    <>
                                      {item.hs125Pointage !== undefined && (
                                        <div className="mt-1 text-xs text-gray-300">
                                          HS 125% Pointage: {item.hs125Pointage.toFixed(2)}
                                        </div>
                                      )}
                                ...
                                {/* HS 150% comparison */}
                                <div className={`p-3 rounded-lg ${
                                  item.hs150Status === 'Correct' ? 'bg-green-900/20' : 'bg-red-900/20'
                                }`}>
                                  <div className="flex items-center">
                                    {item.hs150Status === 'Correct' ? (
                                      <Check className="w-4 h-4 text-green-400 mr-2" />
                                    ) : (
                                      <X className="w-4 h-4 text-red-400 mr-2" />
                                    )}
                                    <span className="font-medium text-sm">
                                      HS 150% Pointage vs HS 50 Paie: 
                                    </span>
                                    <span className={`ml-2 text-sm ${
                                      item.hs150Status === 'Correct' ? 'text-green-400' : 'text-red-400'
                                    }`}>
                                      {item.hs150Status}
                                    </span>
                                  </div>

                                  {item.hs150Pointage === undefined && item.hs50Paie === undefined ? (
                                    <div className="text-xs text-gray-500 italic">
                                      Données HS 150%/50% non disponibles pour cet employé
                                    </div>
                                  ) : (
                                    <>
                                      {item.hs150Pointage !== undefined && (
                                        <div className="mt-1 text-xs text-gray-300">
                                          HS 150% Pointage: {item.hs150Pointage.toFixed(2)}
                                        </div>
                                      )}
                                ...
                                <div className={`p-3 rounded-lg ${
                                  ...
                                  <div className="mt-1 text-xs text-gray-300">
                                    Heures Travaillées: {item.heuresTravaillees?.toFixed(2) || '0.00'}
                                  </div>
                                  <div className="mt-1 text-xs text-gray-300">
                                    Jrs/Hrs Paie: {item.heuresPayees?.toFixed(2) || '0.00'}
                                  </div>
                                  {item.difference !== 0 && (
                                    <div className="mt-1 text-xs text-red-300">
                                      <Info className="inline w-3 h-3 mr-1" />
